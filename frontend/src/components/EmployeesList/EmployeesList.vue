<template>
  <v-data-table :headers="tableHeaders" :items="tableItems" :loading="loading">
    <template #top>
      <v-toolbar>
        ...
        <v-dialog v-model="editDialogOpened" width="400px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn class="mb-2" v-bind="attrs" v-on="on"> Добавить </v-btn>
          </template>
          <template #default>
            <v-card>
              <v-card-title>
              ...
              <v-card-actions>
                ...
                <v-btn text @click="editItem(employeeToEdit)">Сохранить</v-btn>
              </v-card-actions>
            </v-card>
          </template>
        </v-dialog>
        ...
        <v-dialog v-model="deleteDialogOpened" max-width="500px">
          <v-card>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn text @click="deleteItem(employeeToEdit._id)"> Да </v-btn>
...
import { Employee, EmployeePosition, EmployeesSearchResult } from "@/types";
export default defineComponent({
  ...
  setup() {
    ...
    const clearOnCloseEditDialog = watch(editDialogOpened, (newValue) => {
      if (newValue === false) {
        employeeToEdit.value = null;
      }
    });

    const clearOnCloseDeleteDialog = watch(deleteDialogOpened, (newValue) => {
      if (newValue === false) {
        employeeToEdit.value = null;
      }
    });
